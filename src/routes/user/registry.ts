...
router.get("getRegistry", "/", auth, async ctx => {
    const user = await ctx.prisma.user.findUnique({ where: { id: subToId(ctx.state.user.sub) } });
    ctx.status = 200;
...
router.post("setRegistryField", "/set", auth, koaBody(), async (ctx, next) => {
    ...
    Object.keys(ctx.request.body).forEach(f => registry[f] = ctx.request.body[f]);
    const updateClause = Prisma.validator<Prisma.UserUpdateInput>()({
        registry: registry
    });

    await ctx.prisma.$transaction([ctx.prisma.user.update({ where: { id: subToId(ctx.state.user.sub) }, data: updateClause} )]);
    ctx.status = 200;
    ctx.body = `The fields ${Object.keys(ctx.request.body).join(", ")} were updated.`;
});
